...
// CHECK: %[[ABUFFER:.*]] = triton_gpu.alloc
// CHECK: %[[BBUFFER:.*]] = triton_gpu.alloc
...
// CHECK: %[[T_A0:.*]] = triton_gpu.async_shared_copy %{{.*}}, %[[ASUB]] mask %[[LOOP_COND_0_SPLAT_A]] {cache = 1 : i32, evict = 1 : i32, isVolatile = false} : tensor<128x32x!tt.ptr<f16, 1>, #blocked1> -> <128x32xf16, #shared, mutable>
...
// CHECK: %[[T_B0:.*]] = triton_gpu.async_shared_copy %{{.*}}, %[[BSUB]] mask %[[LOOP_COND_0_SPLAT_B]] other %{{.*}} {cache = 1 : i32, evict = 1 : i32, isVolatile = false} : tensor<32x128x!tt.ptr<f16, 1>, #blocked> -> <32x128xf16, #shared1, mutable>
...
// CHECK: %[[T_A1:.*]] = triton_gpu.async_shared_copy %{{.*}}, %[[ASUB1]] mask %[[LOOP_COND_1_SPLAT_A]]
...
// CHECK: %[[T_B1:.*]] = triton_gpu.async_shared_copy %{{.*}}, %[[BSUB1]] mask %[[LOOP_COND_1_SPLAT_B]]
...
// CHECK:   %[[arg_a0_dot_op:.*]] = triton_gpu.shared_load %[[arg_a0]]
// CHECK:   %[[arg_b0_dot_op_0:.*]] = triton_gpu.shared_load %[[arg_b0]]
...
// CHECK:   %[[NEXT_A_BUFFER:.*]] = triton_gpu.async_shared_copy {{.*}}, %[[ASUB3]]
// CHECK:   %[[BSUB3:.*]] = triton_gpu.subview %[[BBUFFER]][%[[INS_IDX_3]], %[[CONSTANT_0]], %[[CONSTANT_0]]]
// CHECK:   %[[NEXT_B_BUFFER:.*]] = triton_gpu.async_shared_copy {{.*}}, %[[BSUB3]]
...
// CHECK:   triton_gpu.async_shared_copy
...
// CHECK:   %[[arg_b0_dot_op:.*]] = triton_gpu.shared_load %[[arg_b0]]
...
// CHECK: triton_gpu.async_shared_copy %[[NEXT_BUFFER_1]]
...
// CHECK: triton_gpu.async_shared_copy %[[NEXT_BUFFER_0]]
...
// CHECK: %[[IND_BUFFER_1:.*]] = triton_gpu.shared_load %[[IND_BUFFER_0]]
...
// CHECK-LABEL: tt.func @load_two_users
  tt.func @load_two_users(%arg0: !tt.ptr<f16, 1> {tt.divisibility = 16 : i32}, %arg1: !tt.ptr<f16, 1> {tt.divisibility = 16 : i32}) -> (tensor<128x16xf32, #mma>, tensor<128x64xf32, #mma>) {
    ...
    %17:2 = scf.for %arg2 = %c0_i32 to %c8_i32 step %c1_i32 iter_args(%arg3 = %cst_1, %arg4 = %cst_2) -> (tensor<128x16xf32, #mma>, tensor<128x64xf32, #mma>)  : i32 {
      ...
      %24 = triton_gpu.alloc %18 : (tensor<64x16xf16, #blocked>) -> !tt.memdesc<64x16xf16, #shared>
      %25 = tt.trans %24 {order=array<i32: 1,0>} : !tt.memdesc<64x16xf16, #shared> -> !tt.memdesc<16x64xf16, #shared1>
      %26 = triton_gpu.shared_load %25 : !tt.memdesc<16x64xf16, #shared1> -> tensor<16x64xf16, #triton_gpu.dot_op<{opIdx = 1, parent = #mma, kWidth = 2}>>
...
module attributes {"triton_gpu.compute-capability" = 80 : i32, "triton_gpu.num-ctas" = 1 : i32, "triton_gpu.num-warps" = 4 : i32, "triton_gpu.threads-per-warp" = 32 : i32} {
  tt.func public @_jagged_hstu_attn_fwd_0d1d2d3d4d5de(%arg0: !tt.ptr<f32, 1> {tt.divisibility = 16 : i32} loc(unknown), %arg1: !tt.ptr<f32, 1> {tt.divisibility = 16 : i32} loc(unknown), %arg2: !tt.ptr<f32, 1> {tt.divisibility = 16 : i32} loc(unknown), %arg3: !tt.ptr<i64, 1> {tt.divisibility = 16 : i32} loc(unknown), %arg4: !tt.ptr<f32, 1> {tt.divisibility = 16 : i32} loc(unknown), %arg5: i32 {tt.divisibility = 16 : i32, tt.max_divisibility = 8 : i32} loc(unknown)) attributes {noinline = false} {
    ...
    %63 = scf.for %arg6 = %c0_i32 to %c64_i32 step %c32_i32 iter_args(%arg7 = %cst) -> (tensor<64x32xf32, #mma>)  : i32 {
      ...
      %72 = triton_gpu.alloc %70 : (tensor<32x64xf32, #blocked1>) -> !tt.memdesc<32x64xf32, #shared>
      %73 = tt.trans %72 {order=array<i32: 1,0>} : !tt.memdesc<32x64xf32, #shared> -> !tt.memdesc<64x32xf32, #shared1>
      %74 = triton_gpu.shared_load %73 : !tt.memdesc<64x32xf32, #shared1> -> tensor<64x32xf32, #triton_gpu.dot_op<{opIdx = 1, parent = #mma, kWidth = 1}>>
