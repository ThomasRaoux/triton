...
#smem = #ttg.shared_memory
// CHECK: #[[$SHARED:.+]] = #ttg.nvmma_shared<{swizzlingByteWidth = 64, transposed = false, elementBitWidth = 16}>
module attributes {"ttg.target" = "cuda:100", "ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, "ttg.threads-per-warp" = 32 : i32} {
  ...
  tt.func @reshape_memedesc(%arg: tensor<2x1x64x64xf16, #blocked>) -> !ttg.memdesc<128x64xf16, #shared, #smem> {
    // CHECK: [[A:.+]] = ttg.local_alloc %{{.+}} : (tensor<2x1x64x64xf16, #{{.*}}>) -> !ttg.memdesc<2x1x64x64xf16, #[[$SHARED]], #smem>
    %r = tt.reshape %arg : tensor<2x1x64x64xf16, #blocked> -> tensor<128x64xf16, #blocked1>
    // CHECK: %[[R:.+]] = ttg.memdesc_reshape %[[A:.+]] : !ttg.memdesc<2x1x64x64xf16, #[[$SHARED]], #smem> -> !ttg.memdesc<128x64xf16, #[[$SHARED]], #smem>
