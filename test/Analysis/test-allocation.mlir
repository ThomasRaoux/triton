...
tt.func @preallocate(%A : !tt.ptr<f16>) {
  ...
  %a = triton_gpu.alloc : () -> !tt.memdesc<32x16xf16, #A_SHARED>
  // CHECK-NEXT: offset = 4096, size = 1024
  %b = triton_gpu.alloc : () -> !tt.memdesc<32x16xf16, #A_SHARED>
  
  triton_gpu.dealloc %cst0 : !tt.memdesc<1x16x16xf16, #A_SHARED>
  // CHECK-NEXT: offset = 0, size = 1024
  %c = triton_gpu.alloc : () -> !tt.memdesc<32x16xf16, #A_SHARED>

  triton_gpu.dealloc %cst1 : !tt.memdesc<1x16x16xf16, #A_SHARED>
  triton_gpu.dealloc %cst2 : !tt.memdesc<1x16x16xf16, #A_SHARED>

  // CHECK-NEXT: offset = 1024, size = 1024
  %cst4 = triton_gpu.alloc : () -> !tt.memdesc<32x16xf16, #A_SHARED>
  // CHECK-NEXT: offset = 6144, size = 2048
  %e = triton_gpu.alloc : () -> !tt.memdesc<64x16xf16, #A_SHARED>
  triton_gpu.dealloc %a : !tt.memdesc<32x16xf16, #A_SHARED>
  // CHECK-NEXT: offset = 8192, size = 2048
  %d = triton_gpu.alloc : () -> !tt.memdesc<64x16xf16, #A_SHARED>
  triton_gpu.dealloc %b : !tt.memdesc<32x16xf16, #A_SHARED>
  // CHECK-NEXT: offset = 10240, size = 2048
  %f = triton_gpu.alloc : () -> !tt.memdesc<64x16xf16, #A_SHARED>
  triton_gpu.dealloc %cst4 : !tt.memdesc<32x16xf16, #A_SHARED>
  triton_gpu.dealloc %c : !tt.memdesc<32x16xf16, #A_SHARED>
  // CHECK-NEXT: offset = 0, size = 2048
  %cst5 = triton_gpu.alloc : () -> !tt.memdesc<64x16xf16, #A_SHARED>
  // CHECK-NEXT: offset = 2048, size = 4096
  %g = triton_gpu.alloc : () -> !tt.memdesc<128x16xf16, #A_SHARED>
  triton_gpu.dealloc %e : !tt.memdesc<64x16xf16, #A_SHARED>
  // CHECK-NEXT: offset = 2048, size = 4096
  %h = triton_gpu.alloc : () -> !tt.memdesc<128x16xf16, #A_SHARED>
  triton_gpu.dealloc %d : !tt.memdesc<64x16xf16, #A_SHARED>
  // CHECK-NEXT: offset = 2048, size = 4096
  %i = triton_gpu.alloc : () -> !tt.memdesc<128x16xf16, #A_SHARED>
  triton_gpu.dealloc %f : !tt.memdesc<64x16xf16, #A_SHARED>
  triton_gpu.dealloc %cst5 : !tt.memdesc<64x16xf16, #A_SHARED>
  tt.return
...
tt.func @unused(%A : !tt.ptr<f16>) {
  ...
  %cst0 = triton_gpu.alloc %cst : (tensor<32x16xf16, #AL>) -> !tt.memdesc<32x16xf16, #A_SHARED>
...
tt.func @longlive(%A : !tt.ptr<f16>) {
  ...
  // CHECK-NEXT: offset = 3072, size = 1024
  %c = triton_gpu.alloc : () -> !tt.memdesc<32x16xf16, #A_SHARED>
  triton_gpu.dealloc %cst3 : !tt.memdesc<1x16x16xf16, #A_SHARED>
  triton_gpu.dealloc %cst4 : !tt.memdesc<1x16x16xf16, #A_SHARED>    
  // CHECK-NEXT: offset = 1024, size = 1024
  %d = triton_gpu.alloc : () -> !tt.memdesc<32x16xf16, #A_SHARED>
  triton_gpu.dealloc %cst0 : !tt.memdesc<1x16x16xf16, #A_SHARED>
  tt.return
...
tt.func @multi_color(%A : !tt.ptr<f16>) {
  ...
  %cst = triton_gpu.alloc : () -> !tt.memdesc<4x8xf16, #A_SHARED>
  // CHECK-NEXT: offset = 1536, size = 32
  %cst_0 = triton_gpu.alloc : () -> !tt.memdesc<4x4xf16, #A_SHARED>
  // CHECK-NEXT: offset = 1664, size = 128
  %cst_1 = triton_gpu.alloc : () -> !tt.memdesc<16x4xf16, #A_SHARED>
  ...
  %1 = triton_gpu.shared_load %cst : !tt.memdesc<4x8xf16, #A_SHARED> -> tensor<4x8xf16, #AL>
  // CHECK-NEXT: offset = 0, size = 128
  %cst_3 = triton_gpu.alloc : () -> !tt.memdesc<4x16xf16, #A_SHARED>
  %2 = triton_gpu.shared_load %cst_0 : !tt.memdesc<4x4xf16, #A_SHARED> -> tensor<4x4xf16, #AL>
  ...
  %cst_4 = triton_gpu.alloc : () -> !tt.memdesc<4x32xf16, #A_SHARED>
  // CHECK-NEXT: offset = 256, size = 64
  %cst_5 = triton_gpu.alloc : () -> !tt.memdesc<4x8xf16, #A_SHARED>
  %4 = triton_gpu.shared_load %cst_5 : !tt.memdesc<4x8xf16, #A_SHARED> -> tensor<4x8xf16, #AL>
  %5 = triton_gpu.shared_load %cst_5 : !tt.memdesc<4x8xf16, #A_SHARED> -> tensor<4x8xf16, #AL>
  // CHECK-NEXT: offset = 1024, size = 512
  %cst_6 = triton_gpu.alloc : () -> !tt.memdesc<8x32xf16, #A_SHARED>
  // CHECK-NEXT: offset = 3104, size = 128
  %cst_7 = triton_gpu.alloc : () -> !tt.memdesc<2x32xf16, #A_SHARED>
  %6 = triton_gpu.shared_load %cst_0 : !tt.memdesc<4x4xf16, #A_SHARED> -> tensor<4x4xf16, #AL>
  // CHECK-NEXT: offset = 1024, size = 512
  %cst_8 = triton_gpu.alloc : () -> !tt.memdesc<1x16x16xf16, #A_SHARED>
  // CHECK-NEXT: offset = 256, size = 32
  %cst_9 = triton_gpu.alloc : () -> !tt.memdesc<4x4xf16, #A_SHARED>
  // CHECK-NEXT: offset = 1024, size = 512
  %cst_10 = triton_gpu.alloc : () -> !tt.memdesc<1x16x16xf16, #A_SHARED>
  %7 = triton_gpu.shared_load %cst_1 : !tt.memdesc<16x4xf16, #A_SHARED> -> tensor<16x4xf16, #AL>
  %8 = triton_gpu.shared_load %cst_4 : !tt.memdesc<4x32xf16, #A_SHARED> -> tensor<4x32xf16, #AL>
  // CHECK-NEXT: scratch offset = 0, size = 1152
  ...
  %10 = triton_gpu.shared_load %cst_7 : !tt.memdesc<2x32xf16, #A_SHARED> -> tensor<2x32xf16, #AL>
...
tt.func @multi_color_multi_rounds(%arg0: !tt.ptr<f16>) {
  ...
  %cst = triton_gpu.alloc : () -> !tt.memdesc<4x4xf16, #A_SHARED>
  // CHECK-NEXT: offset = 1280, size = 128
  %cst_0 = triton_gpu.alloc : () -> !tt.memdesc<16x4xf16, #A_SHARED>
  // CHECK-NEXT: offset = 2048, size = 8192
  %cst_1 = triton_gpu.alloc : () -> !tt.memdesc<1024x4xf16, #A_SHARED>
  %cst_2 = arith.constant dense<0.000000e+00> : tensor<16x32xf16, #AL>
  // CHECK-NEXT: scratch offset = 128, size = 1152
  %0 = triton_gpu.convert_layout %cst_2 : tensor<16x32xf16, #AL> -> tensor<16x32xf16, #AL>
  %1 = triton_gpu.shared_load %cst : !tt.memdesc<4x4xf16, #A_SHARED> -> tensor<4x4xf16, #AL>
  // CHECK-NEXT: offset = 1152, size = 128
  %cst_3 = triton_gpu.alloc : () -> !tt.memdesc<2x32xf16, #A_SHARED>
  %2 = triton_gpu.shared_load %cst : !tt.memdesc<4x4xf16, #A_SHARED> -> tensor<4x4xf16, #AL>
  // CHECK-NEXT: offset = 0, size = 512
  %cst_4 = triton_gpu.alloc : () -> !tt.memdesc<1x16x16xf16, #A_SHARED>
  %3 = triton_gpu.shared_load %cst_0 : !tt.memdesc<16x4xf16, #A_SHARED> -> tensor<16x4xf16, #AL>
  %4 = triton_gpu.shared_load %cst_1 : !tt.memdesc<1024x4xf16, #A_SHARED> -> tensor<1024x4xf16, #AL>
  // CHECK-NEXT: scratch offset = 0, size = 1152
  %5 = triton_gpu.convert_layout %cst_2 : tensor<16x32xf16, #AL> -> tensor<16x32xf16, #AL>
  %6 = triton_gpu.shared_load %cst_3 : !tt.memdesc<2x32xf16, #A_SHARED> -> tensor<2x32xf16, #AL>
...
tt.func @if_else_calls(%A : !tt.ptr<f16>, %cond : i1) {
  ...
  scf.if %cond {
    ...
    %cst0 = triton_gpu.alloc : () -> !tt.memdesc<1x16x16xf16, #A_SHARED>
    // CHECK-NEXT: offset = 0, size = 1024
    %cst1 = triton_gpu.alloc %cst : (tensor<16x32xf16, #AL>) -> !tt.memdesc<16x32xf16, #A_SHARED>
    // CHECK-NEXT: virtual offset = 0, size = 512
    tt.call @alloc1(%A) : (!tt.ptr<f16>) -> ()
  } else {
