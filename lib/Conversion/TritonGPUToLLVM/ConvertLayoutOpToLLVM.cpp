...
private:
  ...
  // store into shared memory using stMatrix
  void processReplicaForStMatrix(
      Location loc, ConversionPatternRewriter &rewriter,
      RankedTensorType type, ArrayRef<unsigned> paddedRepShape,
      ArrayRef<unsigned> origRepShape, ArrayRef<unsigned> outOrd,
      Value llvmSrc, Value smemBase) const {
    storeDistributedToSharedWithStMatrix(type, llvmSrc, smemBase, paddedRepShape,
                                         origRepShape, loc, rewriter);
  }
    ...
    for (unsigned repId = 0; repId < accumNumReplicates; ++repId) {
      ...
      if (srcLayout.isa<BlockedEncodingAttr>() ||
        ...
        if (isStMatrixCompatible(srcTy) && accumNumReplicates == 1) {
          ...
          processReplicaForStMatrix(loc, rewriter, srcTy, paddedRepShape,
                                    origRepShape, outOrd, llvmSrc, smemBase);
        }
        else
  ...
  Value computeStMatrixAddr(Value laneId, int matStride, Location loc,
    ...
    Value matIndex = udiv(
        laneId, i32_val(8)); // linear index of the matrix in the 2x2 matrices
  ...
  void storeDistributedToSharedWithStMatrix(
    ...
    int instrN = 16 * warpsPerCTA[order[0]];
    int instrM = 16 * warpsPerCTA[order[1]];
    std::array<int, 2> numRep = {(int)origRepShape[0] / instrM,
                                 (int)origRepShape[1] / instrN};
    ...
    Value warpIdM = multiDimWarpId[0];
    Value warpIdN = multiDimWarpId[1];

    auto inVals = getTypeConverter()->unpackLLElements(loc, llvmSrc, rewriter);
    // Perform stores.
    Value ldMatrixOffsets =
        computeStMatrixAddr(lane, paddedRepShape[1], loc, rewriter);
    Value relAddr =
        add(ldMatrixOffsets, add(mul(warpIdM, i32_val(16 * paddedRepShape[1])),
                                 mul(warpIdN, i32_val(16))));
    int indexOffset = 0;                                 
    for (int m = 0; m < numRep[0]; m++) {
      for (int n = 0; n < numRep[1]; n++) {

        Value addr =
            add(relAddr, i32_val(n * instrN + m * instrM * paddedRepShape[1]));
        stMatrixm8n8x4(addr, inVals, indexOffset, smemBase, tensorTy.getElementType(), loc,
                       rewriter);
        indexOffset +=  8;                      
      }
    }
