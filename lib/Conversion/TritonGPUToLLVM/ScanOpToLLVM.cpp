...
static void AddPartialReduce(SmallVector<Value> &srcValues,
  ...
  for (unsigned srcIndex = 0; srcIndex < srcValues.size(); srcIndex++) {
    ...
    // Accumulate the partial reduction from shared memory.
    ...
    Value lastElement =
        shflUpSync(loc, rewriter, srcValues[srcIndex], elementStride);
    ...
    for (unsigned i = 1; i < scanElementsPerThreads; ++i) {
      ...
      if (axisBlockId == 0) {
        ...
        // accumulate.
        laneValue =
            select(maskFirstThread, srcValues[srcIndex - i * elementStride], laneValue);
      }
