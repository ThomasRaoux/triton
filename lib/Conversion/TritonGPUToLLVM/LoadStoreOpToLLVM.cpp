...
struct StoreAsyncOpConversion
  ...
  matchAndRewrite(triton::nvidia_gpu::StoreAsyncOp op, OpAdaptor adaptor,
    ...
    Value llPtr = adaptor.getDst();
    Value llShared = adaptor.getSrc();
    auto elemTy = src.getType().cast<RankedTensorType>().getElementType();
    auto smemObj =
...
void mlir::triton::populateLoadStoreOpToLLVMPatterns(
  ...
  patterns.add<StoreAsyncTMAOpConversion>(typeConverter, allocation,
                                          tmaMetadata, tensorPtrMap, benefit);
  patterns.add<StoreAsyncOpConversion>(typeConverter, allocation, axisInfoAnalysis, benefit);
}
