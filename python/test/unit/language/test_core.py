...
@pytest.mark.parametrize("M, N, K, col_a, col_b, rhs_scale, type_a, type_b, num_warps, mma, kpack",
                         [(M, N, K, col_a, col_b, rhs_scale, type_a, type_b, 4, mma, kpack)
...
def test_scaled_dot(M, N, K, col_a, col_b, rhs_scale, type_a, type_b, num_warps, mma, kpack, device):
    if is_cuda():
        cc = torch.cuda.get_device_capability()
        if cc < (8, 9):
            pytest.skip("float8e4nv not supported on CUDA < 8.9")
    ...
    def dot_scale_kernel(a_base, stride_a0, stride_a1, a_scale, b_base, stride_b0, stride_b1, b_scale, out,
        ...
        if b_scale is not None:
            c = tl.dot_scaled(b.T, b_scale, type_b, a.T, a_scale, type_a)
            c = c.T
        else:
    ...
    def dot_scale_ref(x, scale_x, y, scale_y, type_x, type_y):
        def upcast(v, scale, type, transposed):
          ...
          mxfp_to_bf16_kernel[grid](v, scale, v_upcast, scale.numel(), e_bits, m_bits, BLOCK_SIZE, num_warps=num_warps)
    ...
    pgm = dot_scale_kernel[(1, )](x, *x.stride(), scale_x, y, *y.stride(), scale_y, z, M, N, K, type_a, type_b, **kernel_kwargs)
