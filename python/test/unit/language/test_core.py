...
def test_convert2d(M, N, src_layout, interm_layout, dst_layout, dtype, device):
    ...
    """ if interm_layout is None else f"""
    %15 = triton_gpu.alloc %9 : (tensor<{M}x{N}xi32, #src>) -> !tt.memdesc<{M}x{N}xi32, #interm>
    %16 = triton_gpu.shared_load %15 : !tt.memdesc<{M}x{N}xi32, #interm> -> tensor<{M}x{N}xi32, #src>
    %17 = triton_gpu.alloc %11 : (tensor<{M}x{N}xf16, #src>) -> !tt.memdesc<{M}x{N}xf16, #interm>
    %18 = triton_gpu.shared_load %17 : !tt.memdesc<{M}x{N}xf16, #interm> -> tensor<{M}x{N}xf16, #src>
