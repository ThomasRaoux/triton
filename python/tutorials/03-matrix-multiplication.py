...
a = torch.arange(128 * 128, device=DEVICE, dtype=torch.float16).view(128, 128)
b = torch.eye(128, device=DEVICE, dtype=torch.float16)
triton_output = matmul(a, b)
torch_output = torch.matmul(a, b)
torch.set_printoptions(threshold=float('inf'))  # or use profile="full"

print(f"triton_output_with_fp16_inputs={triton_output.to(torch.int16)}")
print(f"torch_output_with_fp16_inputs={torch_output.to(torch.int16)}")
