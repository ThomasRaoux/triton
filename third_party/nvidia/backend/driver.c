...
static PyObject *fill2DTMADescriptor(PyObject *self, PyObject *args) {
  ...
  int rank = 2;
  // Swizzling should be picked in codegen but since we need to set it on the descriptor we rely on
  // a convention between this function and codegen.
  ...
  // The bounding box inner dimension must be less than or equal to the swizzle size.
